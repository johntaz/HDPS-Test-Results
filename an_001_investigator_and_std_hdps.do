/*==============================================================================
FILE NAME:				an_001_investigator_and_std_hdps
AUTHOR:					John Tazare
DESCRIPTION OF FILE:	Run investigator and standard HDPS analysis (i.e. no testing)
*=============================================================================*/
...
* Results
tempname john 
postfile `john' str30(outcome) str20(bias) float(no_of_vars hr ll ul) using "$projectdir/data/testResults_investigator_std_hdps", replace 
...
keep if imd !=. // may do sensitivity analysis around excluding/including these 733
assert _N == 858295
...
// Have already redefined these so just need to rename
assert alclevel!=.
assert alcstatus!=.
assert smokstatus !=.
...
logistic ppi_exposure (${vars})##i.calendar_year_cat 
...
post `john' ("lld_COPD") ("investigator") (0) (`hr') (`ll') (`ul') 
 
keep patid propensity ppi_exposure 
save "$projectdir/data/pscores/pscores_investigator.dta", replace
...
use "$projectdir/reviewercomments-2026/data/hdps_covariates.dta", replace
...
keep patid propensity ppi_exposure 
save "$projectdir/data/pscores/pscores_std_`no_overall'.dta", replace
